...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = (
            "You are an expert at generating deep epistemic personas for real researchers. "
            "Given a researcher's name, discipline, and paper abstracts, produce a persona that "
            "captures their intellectual identity for a cross-disciplinary debate simulation. "
            "Return valid JSON only. No newlines inside string values."
        )
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate an epistemic research persona for the following researcher for use in a cross-disciplinary debate simulation.

Researcher: {entity_name}
Discipline: {entity_type}
...
1. bio: A 1-2 sentence academic bio suitable for a conference profile (max 200 chars).
2. persona: A detailed epistemic persona (~500 words of plain text). Must include:
   - Core beliefs: what fundamental assumptions drive their research
   - Preferred methodology: how they argue and what they count as evidence
   - Intellectual blind spots: what they tend to dismiss or overlook
   - Key positions: their most distinctive/controversial scientific stances
   - Debate style: how they respond to challenges (combative, conciliatory, empirical...)
   - Grounding: the key claims from their published abstracts above
3. age: integer (estimated or real)
4. gender: "male", "female", or "other" (English only)
5. mbti: MBTI type reflecting their intellectual style (e.g. INTJ for systematic theorists)
6. country: country of main institution (English)
7. profession: their specific research role (e.g. "Computational Neuroscientist")
8. interested_topics: array of 3-6 research topics they care about most

Rules:
- All string values on single lines (no embedded newlines)
- age must be a valid integer, gender must be "male"/"female"/"other"
- Base the persona tightly on the abstracts provided — this is their corpus grounding
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a research-institution epistemic profile for use in a cross-disciplinary debate simulation.
...
1. bio: A 1-2 sentence description of the institution's research focus (max 200 chars).
2. persona: A concise profile (~300 words plain text) covering: research mission, key disciplines, institutional stance on major scientific questions, how it engages with interdisciplinary debate.
3. age: 30 (fixed for institutional accounts)
4. gender: "other" (fixed for institutions)
5. mbti: MBTI type reflecting the institution's intellectual culture
6. country: country (English)
7. profession: research domain description
8. interested_topics: array of 3-6 key research areas
